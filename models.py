...
def insert_data_canadian_website(data=None):
    ...
    with sess.connection() as con:
        ...
        statement = text("INSERT INTO travel_db.canadian_alert_info (country_name, alert_text, last_updated, risk_heading, risk_information, country_security, criteria, health, laws, natural_disaster, canadian_key_encode ) VALUES (:country_name, :alert_text, :last_updated, :risk_heading, :risk_information, :country_security, :criteria, :health, :laws, :natural_disaster, :canadian_key_encode) ON DUPLICATE KEY UPDATE alert_text = alert_text, last_update = last_update, risk_heading = risk_heading, risk_information = risk_information, country_security = country_security, criteria = criteria, health = health")
        con.execute(statement, data)
        print("please wait inserting data")